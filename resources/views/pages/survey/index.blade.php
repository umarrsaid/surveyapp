...
@push('title','Project')
@push('main_title','<i class="fa fa-chevron-right"></i> Project')
                            ...
                            <table class="" id="tabel_data_survey" >
                              <thead>
                                 <tr>
                                    ...
                                    <th>Nama</th>
                                    <th>Durasi Menit</th>
                                    <th class="text-center">Soal Responden</th>
                                    <th>Status</th>
                                    <th>Pertanyaan</th>
                                 </tr>
                              </thead>
...
<script type="text/javascript">
    ...
    $(document).delegate('select', 'change', function(event){
        if ($(this).val() == 'pilihan') {
            $( this ).closest('.det-row').find("textarea").prop('readonly',false);
        }else{
            $( this ).closest('.det-row').find("textarea").prop('readonly',true);
        }
    });

    $(document).delegate('.check', 'click', function(event){
        if ($(this).is(':checked')) {
            $( this ).closest('.det-row').find("input[name='is_required[]']").val(1);
        }else{
            $( this ).closest('.det-row').find("input[name='is_required[]']").val(0);
        }
    });
    ...
    $(document).on("click", ".add-det", function() {
        ...
        $('.det-row:last input[name="is_required[]"]').val(0);
        $('.det-row:last textarea').val('-');
        $('.det-row:last input:checkbox').prop("checked", false);
        $('.det-row:last input[name="id_soal_resp[]"]').remove();
    });
    ...
    function tabel_responden(){
        ...
        data_col = [
                ...
                {data:'nama',width: '60%'},
                {data:'durasi',width: '100', class:'text-center'},
                {data:'soal_resp',width: '50', class:'text-center',searchable:false},
                {data:'editable',width: '100',searchable: false,class:'text-center'},
                {data:'pertanyaan',searchable: false,width: '100', class:'text-center'},
        ];
...
$(document).on('click', '#hapus', function() {
    ...
    // alert(c);
    bootbox.dialog({
        title: '<span class="blue"><i class="fa fa-trash"></i> Hapus</span> Project ini',
        message: "Anda yakin akan menghapus Project ini ?",
        buttons: {
            confirm: {
